...
# ============================================================
# UNIFIED SCAN AREA SETTINGS
# ============================================================

# Right-side scanning area — same for both candle and number detection
...
# ============================================================
# ============================================================
# CANDLE DETECTION (PURPLE / YELLOW)
# ============================================================
# ============================================================
...
def detect_candles(img):
    ...
    roi, offset_x, offset_y = get_right_side_roi(img)
    ...
    # Sort RIGHT → LEFT (newest first)
    final_candles.sort(key=lambda c: c["center_x"], reverse=True)

    return final_candles


def create_candle_report(candles):
    ...
    for i, candle in enumerate(candles, 1):
        ...
        sequence.append(f"{i}. {color} {candle['color']} {verified} ({score:.0f}%)")
...
def find_components(mask):
    ...
    for contour in contours:
        ...
        if h < MIN_DIGIT_HEIGHT or h > MAX_DIGIT_HEIGHT:
            continue
        if w < MIN_DIGIT_WIDTH or w > MAX_DIGIT_WIDTH:
            continue
        if w > h * 3.5:
            continue
...
def analyze_screenshot(image_path):
    ...
    candle_report = create_candle_report(candles)
    ...
    return combined_analysis, img, candles, number_results
...
def create_unified_detection_map(img, candles, number_results):
...
def handle_photo(message):
    ...
    try:
        ...
        combined, img, candles, numbers = analyze_screenshot(image_path)
